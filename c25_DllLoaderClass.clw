...
c25_DllLoaderClass.Construct                                        Procedure()
ClassStarter &c25_ClassStarter
Code

    Self.ClassTypeName = 'c25_DllLoaderClass'
    ...
    If Records(Self.ProductsQ) = 0
        Self.ProductsQ.DllName  = 'QueueMeta001.dll'
        Self.ProductsQ.PathDll  = 'QueueMeta001.dll'
        Self.ProductsQ.Name     = 'QueueExtender'
        Add(Self.ProductsQ)

        Self.ProductFunctionsQ.FunctionName = 'CreateQueueDll'
        Self.ProductFunctionsQ.ProductName = 'QueueExtender'
        Self.ProductFunctionsQ.Ordinal = 1
        Add(Self.ProductFunctionsQ)
    End

c25_DllLoaderClass.Destruct                                         Procedure()
...
c25_DllLoaderClass.GetModuleHandle                                  Procedure(<string _productName>)
...
Code
    ...
    LOOP
        ...
        If Errorcode() <> 0
            BREAK
        End
        If Lower(Clip(Self.ProductsQ.Name)) = Lower(Clip(_productName))
            Return Self.ProductsQ.DllLibraryHandle
        End
    End
    Return 0

c25_DllLoaderClass.GetMessageNameById                               Procedure(Long _id)
...
c25_DllLoaderClass.FreeResources                                    Procedure()
...
Code
    ...
    LOOP
        ...
        If Self.ProductsQ.DllLibraryHandle <> 0
            Loop
                B# = c25_FreeLibrary(Self.ProductsQ.DllLibraryHandle)
                If B# = 0
                    BREAK
                End
            End
            ...
            LOOP
                ...
                If Errorcode() <> 0
                    BREAK
                End
                If Self.ProductFunctionsQ.ProductName = Self.ProductsQ.Name
...
c25_DllLoaderClass.LoadDllIntoMemory                                Procedure(<String _dllNameA>, <String _dllNameW>, <string _productName>)
    ...
    Code
        ...
        Loop
            ...
            If Errorcode() <> 0
                Break
            End
            If Upper(Clip(Self.ProductsQ.Name)) = Upper(Clip(_productName))
                If Self.ProductsQ.DllLibraryHandle = 0
                    If omitted(_dllNameW) = True
                        Self.ProductsQ.DllLibraryHandle = Self.LoadDllLibrary(_dllNameA,)
                    Else
                        Self.ProductsQ.DllLibraryHandle = Self.LoadDllLibrary(, _dllNameW)
                    End
                    Put(Self.ProductsQ)

                    If Self.ProductsQ.DllLibraryHandle = 0
                        Message('Error 1 loading c25_DllLoaderClass.LoadDllFunctionCall Dll: ' & _dllNameA)
                        break
                    End
                    Return Self.ProductsQ.DllLibraryHandle
                End
...
c25_DllLoaderClass.LoadDllFunctionCall                              Procedure(String _dllNameA, <String _functionName>, <Long _ordinal>, <String _dllNameW>, <string _productName>)

Found_ProductsQ Bool
            ...
            If Upper(Clip(Self.ProductsQ.Name)) = Upper(Clip(_productName))
                ...
                Loop
                    ...
                    If Omitted(_functionName) = False
                        If Upper(Clip(Self.ProductFunctionsQ.FunctionName)) <> Upper(Clip(_functionName))
                            CYCLE
                        End
                    ELSE
                        If Self.ProductFunctionsQ.Ordinal <> _ordinal
                            CYCLE
                        End
                    End
                    If Self.ProductFunctionsQ.ProcHandle <> 0
                        Return Self.ProductFunctionsQ.ProcHandle
                    End

                    If Self.ProductsQ.DllLibraryHandle = 0
                        Self.ProductsQ.DllLibraryHandle = Self.LoadDllIntoMemory(_dllNameA,,_productName)
                        Put(Self.ProductsQ)
                    End

                    If Omitted(_functionName) = False
                        Self.ProductFunctionsQ.ProcHandle = Self.GetProcAddress(Self.ProductsQ.DllLibraryHandle, _functionName,)
                    Else
                        Self.ProductFunctionsQ.ProcHandle = Self.GetProcAddress(Self.ProductsQ.DllLibraryHandle, , _ordinal)
                    End
                    Put(Self.ProductFunctionsQ)
                    Return Self.ProductFunctionsQ.ProcHandle
                    Break
...
c25_DllLoaderClass.LoadDllLibrary                                   Procedure(<String _dllNameA>, <String _dllNameW>)

c25DllFilePath      String(1024)
DllLibraryHandle    Long

    Code

        If omitted(_dllNameA) = False
            c25DllFilePath = Clip(Left(_dllNameA)) & Chr(0)

            DllLibraryHandle = c25_LoadLibraryA(Address(c25DllFilePath))
            IF DllLibraryHandle = 0
                Message('error loading DllLibraryHandle ' & c25DllFilePath)
            End
        Else
            c25DllFilePath = Clip(Left(_dllNameW)) & Chr(0) & Chr(0)
            DllLibraryHandle = c25_LoadLibraryW(Address(c25DllFilePath))
        End
        Return DllLibraryHandle

c25_DllLoaderClass.GetProcAddress                                   Procedure(Long _dllHandle, <String _procedureName>, <Long _ordinal>)

ProcedureName           String(1024)
ProcedureAddress        Long

    Code

        If Omitted(_procedureName) = False
            ProcedureName       = Clip(Left(_procedureName)) & Chr(0)
            ProcedureAddress    = c25_GetProcAddress(_dllHandle, Address(ProcedureName))
        Else
            ProcedureAddress    = c25_GetProcAddress(_dllHandle, _ordinal)
        End
        Return ProcedureAddress

c25_DllLoaderClass.CallDllFunction                                  Procedure(String _functionName, <String _dllNameA>, <String _params>, <*Long returnVal>)

    Code

    Return 0

c25_DllLoaderClass.CallDllFunction                                  Procedure(String _functionName, <String _dllNameA>, <String _params>, <*String returnVal>)

    Code

    Return ''

c25_DllLoaderClass.CallDllFunction                                  Procedure(String _functionName, <String _dllNameA>, <String _params>, <*queue returnVal>)

    Code

        Return Null

c25_DllLoaderClass.NotYetImplemented                                Procedure()
